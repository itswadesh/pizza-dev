...
var sauceRadios = document.querySelectorAll('input[name="sauce"]');
sauceRadios.forEach(function (radio) {
  radio.checked = false;
});

var cheeseRadios = document.querySelectorAll('input[name="cheese"]');
cheeseRadios.forEach(function (radio) {
  radio.checked = false;
});
  ...
  function getSelectedRadioText(name) {
    ...
    for (var i = 0; i < radios.length; i++) {
      if (radios[i].checked) {
        return radios[i].getAttribute("data-size");
      }
    }
    return '';
  }
